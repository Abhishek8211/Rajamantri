...
function createComputerPlayer(
  room,
  index,
  personality = BOT_PERSONALITIES.SMART
) {
  ...
  setTimeout(() => {
    if (room.currentRound < room.rounds) {
      ...
      const winnerMessage = {
        id: Date.now().toString(),
        username: "System",
        message: `Game Over! ${winner.username} wins with ${
          room.scores[winner.id]
        } points! 🎉`,
        type: "system",
        timestamp: new Date().toLocaleTimeString(),
        playerId: "system",
...
io.on("connection", (socket) => {
  ...
  socket.on("start-game", (roomCode) => {
    ...
    // Check if we have at least 4 players total (real + bots)
    if (room.players.length < 4) {
      const errorMessage = `At least 4 players (including bots) are required to start the game. Currently: ${room.players.length}/4`;
      ...
      console.log(
        `❌ Cannot start game in room ${roomCode}: only ${room.players.length} players`
      );
      return;
    }
    ...
    console.log(
      `🚀 Game started in room ${roomCode} with ${room.players.length} players`
    );
  });
  ...
  socket.on("mantri-call-sipahi", (roomCode) => {
    ...
    if (room.gameState !== "guessing") {
      console.error(
        '❌ Wrong game state! Expected "guessing", got:',
        room.gameState
      );
      return;
    }
  ...
  // Update bot settings
  socket.on("update-bot-settings", (data) => {
    const { roomCode, addBots, botCount, botDifficulty } = data;
    const room = rooms.get(roomCode);

    if (room && room.host === socket.id) {
      room.addBots = addBots;
      room.botCount = botCount || 0;
      room.botDifficulty = botDifficulty || "smart";
      ...
      const systemMessage = {
        ...
        username: "System",
        message: addBots
          ? `Host configured ${botCount} AI player(s) to join on game start`
          : "Host removed AI players",
        type: "system",
        timestamp: new Date().toLocaleTimeString(),
        playerId: "system",
      };
      ...
      console.log(
        `🤖 Bot settings updated in room ${roomCode}: ${botCount} bots (${botDifficulty})`
      );
    }
  });
  ...
  socket.on("remove-player", (data) => {
    ...
    if (playerId === socket.id) {
      socket.emit("error", "You cannot remove yourself");
      return;
    }
    ...
    const systemMessage = {
      id: Date.now().toString(),
      username: "System",
      message: `${playerToRemove.username} was removed from the lobby by the host`,
      type: "system",
      timestamp: new Date().toLocaleTimeString(),
      playerId: "system",
    };
    ...
    // Broadcast updated room to all remaining players
    io.to(roomCode).emit("room-updated", room);

    console.log(
      `👋 ${playerToRemove.username} removed from room ${roomCode} by host`
    );
  });
...
function mantriCallSipahi(room, mantri) {
  ...
  io.to(room.code).emit("mantri-called-sipahi", {
    mantriId: mantri.id,
    sipahiId: sipahi.id,
  });
...
function sipahiGuess(room, sipahi, guessedPlayerId) {
  ...
  io.to(room.code).emit("sipahi-guessed", {
    sipahiId: sipahi.id,
    guessedPlayerId,
  });
...
function processGuess(room, guessedPlayerId) {
  ...
  if (isCorrect) {
    resultMessage =
      `Correct! ${guessedPlayer.username} was the Chor! 🎯\n` +
      `Raja: +1000, Mantri: +800, Sipahi: +500, Chor: 0`;
  } else {
    resultMessage =
      `Wrong! ${guessedPlayer.username} was not the Chor. The real Chor was ${chor.username}!\n` +
